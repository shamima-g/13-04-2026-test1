...
- [x] AC-1: Given I am signed in as a team-member and I have a pending task assigned to me, when I view that task, then I can see a "Mark Complete" control.
- [x] AC-2: Given I am signed in as a team-member and a task assigned to me is already complete, when I view that task, then I do not see a "Mark Complete" control.
- [x] AC-3: Given I am signed in as an admin, when I view any task, then I do not see a "Mark Complete" control on any task.

### Loading Indicator (R12)

- [x] AC-4: Given I click "Mark Complete" on a pending task assigned to me, when the request is sent to the API, then a loading indicator is shown on that task's control while the request is in flight.
- [x] AC-5: Given the "Mark Complete" request is in flight, when I look at the task, then the task's status has not yet changed in the UI — the update only happens after the API responds.

### Successful Completion (R12, R13)

- [x] AC-6: Given I clicked "Mark Complete" and the API confirms the change, when the response arrives, then the task's status updates to "Complete" in the UI.
- [x] AC-7: Given the task has been marked complete, when I look at the task, then the "Mark Complete" control is no longer shown.
- [x] AC-8: Given the task has been marked complete and I have a status filter active, when the task status updates, then the list correctly reflects the new status (for example, the task disappears from "Pending" filter and appears under "Complete" filter).

### No Revert Allowed (BR8)

- [x] AC-9: Given a task has status "Complete", when I view it as any user, then there is no control to change it back to "Pending".

### Error Handling (R12)

- [x] AC-10: Given I clicked "Mark Complete" and the API call fails, when the failure occurs, then an error message is shown indicating the update did not go through.
- [x] AC-11: Given the "Mark Complete" API call failed, when the error is shown, then the task's status remains unchanged — it still shows as pending.
- [x] AC-12: Given the error message is shown after a failed "Mark Complete", when the error is dismissed or I retry, then the "Mark Complete" control is restored so I can try again.
